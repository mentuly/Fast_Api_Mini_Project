    ...
    def check_model(self):
        if self.start_date >= self.end_date:
            validations_logger.info(
                "Model: DateRange, Field: start_date, Result: Failed (start date is not before end date)"
            )
            raise HTTPException(
                status_code=400, detail="Start date should be before end date"
            )
        if self.end_date >= datetime.now():
            validations_logger.info(
                "Model: DateRange, Field: end_date, Result: Failed (end date in future)"
            )
